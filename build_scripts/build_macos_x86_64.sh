...
# Builds Boost and libtorrent for macOS (Intel) using CMake
#
...
# ── 1. build Boost for macOS x86_64 ────────────────────────────────
echo "▶︎ Building Boost for macOS x86_64..."

if [ ! -f "$BOOST_STAGE_LIB/libboost_system.a" ]; then
    (cd "$BoostSrc" && \
        "$B2" -j"$Jobs" \
          toolset=clang \
          target-os=darwin architecture=x86 address-model=64 \
