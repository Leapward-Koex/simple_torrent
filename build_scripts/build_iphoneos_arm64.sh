...
#
# iOS Device arm64 build script
# Builds Boost and libtorrent for iOS device (arm64) using CMake
#
...
BuildRoot="${ToolsDir}/build-ios-device-${ARCH}"
...
# ── 1. build Boost for iOS device arm64 ────────────────────────────────
echo "▶︎ Building Boost for iOS Device arm64..."
...
if [ ! -f "$BOOST_STAGE_LIB/libboost_system.a" ]; then
    (cd "$BoostSrc" && \
        "$B2" -j"$Jobs" \
          ...
          cflags="-arch ${ARCH} -isysroot ${IOS_SDK_PATH} -mios-version-min=${MIN_IOS_VERSION}" \
          cxxflags="-arch ${ARCH} -isysroot ${IOS_SDK_PATH} -mios-version-min=${MIN_IOS_VERSION}" \
          linkflags="-arch ${ARCH} -isysroot ${IOS_SDK_PATH} -mios-version-min=${MIN_IOS_VERSION}" \
