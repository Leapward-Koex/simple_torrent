<#
Build Boost (static, PIC) and libtorrent (static) for Android x86_64.
...
$BoostOut   = Join-Path $ToolsDir 'boost-android-x86_64'
$LibtPrefix = Join-Path $ToolsDir 'libtorrent-x86_64'
...
$Clang      = Join-Path $Toolchain 'bin\x86_64-linux-android24-clang++.cmd'
...
$Clang = $Clang.Replace('\','/')
$Ar    = $Ar.Replace('\','/')
$Jam = Join-Path $ToolsDir 'android-x86_64.jam'
@"
using clang : android
    : "$Clang"
    : <compileflags>"-fPIC"
      <linkflags>"-fPIC"
      <arch>x86        <address-model>64
      <abi>sysv           <binary-format>elf
      <target-os>android
      <archiver>"$Ar"
;
